...
// THIS FILE BELONGS TO: kernel/src/fs/fat.cpp
...
// ADDED: Implementation for fat_directory_file::pread
size_t fat_directory_file::pread(void *buffer, size_t offset, size_t length) {
	// Load children
	node_.load();
	unsigned int child_index = offset / sizeof(stacsos::dirent);
	
	// Check whether the end of the directory has been reached
	if (child_index >= node_.children_.count()) {
		return 0;
	}

	// Get the child node at that index
	fat_node *child = node_.children_.at(child_index);

	// Create a dirent struct -> copy it to the user's buffer
	stacsos::dirent entry;
	memops::memset(&entry, 0, sizeof(entry));
	memops::memcpy(entry.name, child->name().c_str(), child->name().length());

	// Map kernel node kind to our simple integer kind
	if (child->kind() == fs_node_kind::directory) {
		entry.kind = 1;
	} else {
		entry.kind = 0;
	}

	entry.size = child->data_size_;

	// Copy the single entry to the user buffer and return the bytes read
	size_t bytes_to_copy = min(length, sizeof(stacsos::dirent));
	memops::memcpy(buffer, &entry, bytes_to_copy);
