...
// THIS FILE BELONGS TO: kernel/inc/stacsos/kernel/fs/fat.h
...
// ADDED: Forward declaration for fat_node
class fat_node;
...
// ADDED: Class for reading directory listings
class fat_directory_file : public file {
public:
	// Pass in the directory node
	fat_directory_file(fat_node &node) : file(~0ULL), node_(node) { }
	virtual ~fat_directory_file() { }

	// Called by "read()" in user/ls/src/main.cpp
	virtual size_t pread(void *buffer, size_t offset, size_t length);
	virtual size_t pwrite(const void *buffer, size_t offset, size_t length) {
		return 0;
	}
...
class fat_node : public fs_node {
	...
	friend class fat_directory_file;
...
public:
	...
	// MODIFIED: Added new case for directory 
	...
	virtual shared_ptr<file> open() override {
		if (kind() == fs_node_kind::file) {
			return shared_ptr<file>(new fat_file((fat_filesystem &)fs(), cluster_, data_size_));
		} else {
			return shared_ptr<file>(new fat_directory_file(*this));
		}
	}
